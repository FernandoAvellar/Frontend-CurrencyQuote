...
export default function Admin() {
  ...
  const handleChangePassword = async (username) => {
    ...
    try {
      await api.put(`/users/${username}/password/change`, {
  ...
  const handleUpdateRoles = async (username) => {
    const selectedRoles = Object.keys(roleSelections[username]).filter(
      (role) => roleSelections[username][role]
    );
                ...
                <div className="flex justify-center">
                  <label className="mr-4">
                    <input
                      ...
                      onChange={() =>
                        handleRoleChange(u.username, "ROLE_BASIC")
                      }
                    />
                    ROLE_BASIC
                  </label>
                  <label>
                    <input
                      ...
                      onChange={() =>
                        handleRoleChange(u.username, "ROLE_ADMIN")
                      }
                    />
                    ROLE_ADMIN
                  </label>
