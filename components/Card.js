    ...
    <div className="border rounded-lg shadow-lg text-white bg-sky-800 max-w-md min-w-full">
      <div className="rounded-t-lg h-16 p-1 mb-2 bg-gray-800 text-center place-content-center">
        <h2 className="text font-semibold">
          {currency} - {rate ? rate.currency.name : "Loading..."}
        </h2>
      </div>
      <div>
        <p className="text-xl p-2 text-center place-content-center">
          {rate
            ? rate.bid.toLocaleString("pt-BR", {
                style: "currency",
                currency: "BRL",
        ...
        {rate && (
          <div className="flex flex-row itens-center justify-center mb-2">
            <p className="p-2">
              High:{" "}
              {rate.high.toLocaleString("pt-BR", {
                style: "currency",
                currency: "BRL",
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
      ...
      <div className="rounded-b-lg h-10 bg-gray-800 text-center place-content-center">
        <p>
          Last update:{" "}
          {rate
            ? new Date(rate.createDate).toLocaleString("pt-BR", {
                dateStyle: "short",
                timeStyle: "medium",
              })
            : "Loading..."}
